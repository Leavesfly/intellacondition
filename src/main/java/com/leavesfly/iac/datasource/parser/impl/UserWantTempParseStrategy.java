 ...
 * 用户期望温度解析策略实现类
 * 
 * 该类负责解析用户期望温度数据，返回用户ID和期望温度的键值对。
 * 数据格式：用户ID\t期望温度
 * 
 * 示例：0\t23
 */
public class UserWantTempParseStrategy implements DataParseStrategy<Map.Entry<String, Float>> {
    ...
    @Override
    public Map.Entry<String, Float> parse(String dataLine) throws ParseException {
        if (StringUtils.isBlank(dataLine)) {
            throw new ParseException("用户期望温度数据行不能为空", dataLine, -1, getStrategyType());
        }
        
        String trimmedLine = dataLine.trim();
        
        if (!validate(trimmedLine)) {
            throw new ParseException("用户期望温度数据格式不正确", trimmedLine, -1, getStrategyType());
        }
        ...
        try {
            ...
            return new AbstractMap.SimpleEntry<>(userId, wantTemp);
            
        } catch (ParseException e) {
            throw e;
        } catch (Exception e) {
            throw new ParseException("解析用户期望温度数据时发生未知错误: " + e.getMessage(), 
    ...
    public String getFormatDescription() {
        return "用户期望温度数据格式: 用户ID\\t期望温度";
    }
    
    @Override
    public String getStrategyType() {
        return "USER_WANT_TEMP";
    }
}